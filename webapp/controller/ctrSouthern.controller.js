sap.ui.define([
    ...
    function (Controller, MessageBox, BusyIndicator) {
        ...
        return Controller.extend("nmspoplog.oplog.controller.southern", {
            onInit: async function () {
                var oView = this.getView("southern");              //Instance of view
                var oModel = new sap.ui.model.json.JSONModel(); //This model  will contain the data
                var dataModel = this.getOwnerComponent().getModel("dataSt");//Will contain data mapped from the ODATA services
                ...
                if (dataModel.oData.init !== "X") {         //This will only execute the first time the view is displayed
                    ...
                    oRouter.getRoute("RouteSt").attachMatched(this._onRouteMatched, this);
                }
                ...
                if (usrSrv[0].result === "ERROR") {
                    ...
                    if (WcdSrv[0].result === "ERROR") {
                        ...
                        if (PusRepSrv[0].result === "ERROR") {
                            ...
                            var srvRmrksUrl = "/sap/opu/odata/sap/ZODATA_MC_MULTDIV_REMARK_OPLOG_SRV/";
                            var OdataSrvRmrks = new sap.ui.model.odata.ODataModel(srvRmrksUrl, true);
                            var RmrksData = "";
                            var rmrksSrv = await this.getRmrksSrv(OdataSrvRmrks, RmrksData, dataModelMain);
                            if (rmrksSrv[0].result === "ERROR") {
                                sap.ui.core.BusyIndicator.hide();
                                MessageBox.error((rmrksSrv[0].data));
                            }
                            else {
                                var srvMidNgthConUrl = "/sap/opu/odata/sap/ZODATA_MC_MULTDIV_OPLOG_SRV/";
                                var OdataSrvData = new sap.ui.model.odata.ODataModel(srvMidNgthConUrl, true);
                                var MidNgthCon = "";
                                var MidNgthConSrv = await this.getMidNgthConSrv(OdataSrvData, RmrksData,dataModelMain);
                                if (MidNgthConSrv[0].result === "ERROR") {
                                    ...
                                    MessageBox.error((MidNgthConSrv[0].data));
                                     ...
                                     var srvOpLog = "/sap/opu/odata/sap/ZODATA_MC_MULTDIV_TABLE_OPLOG_SRV/";
                                        ...
                                        var MidNgthCon = "";
                                        var OpLogSrv = await this.getOpLog(OdataSrvData, RmrksData, dataModelMain);
                                        ...
                                        //We need to initialize the dataModel in case there is data from another execution
                                            ...
                                            for (let element in dataModel.oData) {
                                                var property = "/";
                                                if (element !== "init" && element !== "initidx" && element.substring(0,3) !== "lst" && element !== "valDate") {
                                     ...
                                     if (MidNgthConSrv[0].data.results.length === 0) {
                                            ...
                                            oView.byId("chk_DcppIso1").setSelected(false);
                                            oView.byId("chk_DcppIso2").setSelected(false);
                                            oView.byId("chk_DcppIso3").setSelected(false); 
                                            oView.byId("chk_DcppIso4").setSelected(false);
                                        }
                                        else {
                                            
                                            dataModel = this.SetData(usrSrv, WcdSrv, PusRepSrv, rmrksSrv, MidNgthConSrv, today, dataModel,"",formResult)
                                            //oModel.setData(null);
                                            oModel = dataModel;
                                            oView.setModel(oModel);
                                            sap.ui.core.BusyIndicator.hide();
...
//#endregion onFocusOut
            ...
            getOpLog: async function (UsrModel, data, dataModelMain) {
            ...
            getRmrksSrv: async function (UsrModel, data, dataModelMain) {
                //Esto sirve para saber si una variables ya esta definida
                var resolve = "";
                var reject = "";
                const oPromise = await new Promise(function (resolve, reject) {
                    UsrModel.read("/Southern", {
                        urlParameters: {
                            "Southerndate": dataModelMain.oData.valParamDate
                        },
                        success: (oData) => {
                            //alert(oData.results[0].Accm);
                            //alert(oResponse);
            ...
            getMidNgthConSrv: async function (UsrModel, data, dataModelMain) {
                //Esto sirve para saber si una variables ya esta definida
                var resolve = "";
                var reject = "";
                const oPromise = await new Promise(function (resolve, reject) {
            ...
            SetData: function (userData, wcdData, plantData, rmrksData, MidCondData, date, model, McDate,formResult) {

                var oView = this.getView("southern");
                var dataModel = model;
                var ctrllr = oView.getController();
                var obj = "";
                var UsrData = userData[0].data.results[0];
                var logDate = MidCondData[0].data.results[0].Mcdate.substring(4, 6) + "/" + MidCondData[0].data.results[0].Mcdate.substring(6, 8) + "/" + MidCondData[0].data.results[0].Mcdate.substring(0, 4);
                dataModel.setProperty("/valDate", logDate);
                
                dataModel.setProperty("/valOpName", MidCondData[0].data.results[0].Uname);
                ...
                switch (MidCondData[0].data.results[0].Natsec) {
                    ...
                    case "IMMINENT":
                        ...
                        break;
                    case "GREEN":
                        dataModel.setProperty("/valSecThrLvNat", "keyStlNat4");
                        iconSec.setBackgroundColor("");
                        iconSec.setColor("");
                        break;
                    case "BLUE":
                        dataModel.setProperty("/valSecThrLvNat", "keyStlNat5");
                        iconSec.setBackgroundColor("");
                        iconSec.setColor("");
                        break;
                    case "YELLOW":
                        dataModel.setProperty("/valSecThrLvNat", "keyStlNat6");
                        iconSec.setBackgroundColor("");
                        iconSec.setColor("");
                        break;
                    case "ORANGE":
                        dataModel.setProperty("/valSecThrLvNat", "keyStlNat7");
                        iconSec.setBackgroundColor("");
                        iconSec.setColor("");
                        break;
                    case "RED":
                        dataModel.setProperty("/valSecThrLvNat", "keyStlNat8");
                        iconSec.setBackgroundColor("");
                        iconSec.setColor("");
                        break;
                    default:
                ...
                switch (MidCondData[0].data.results[0].Dwrsec) {
                    ...
                    case "IMMINENT":
                        ...
                        break;
                    case "1":
                        dataModel.setProperty("/valSecThrLvDwr", "keyStlDwr4");
                        ...
                        break;
                    case "2":
                        dataModel.setProperty("/valSecThrLvDwr", "keyStlDwr5");
                        iconDWR.setBackgroundColor("");
                        iconDWR.setColor("");
                        //var lbl = view.byId("lbl_SecThreatLvls_dwr_color");
                        //lbl.addStyleClass("cbx_red");
                        break;
                    case "3":
                        dataModel.setProperty("/valSecThrLvDwr", "keyStlDwr6");
                        iconDWR.setBackgroundColor("");
                        iconDWR.setColor("");
                        //var lbl = view.byId("lbl_SecThreatLvls_dwr_color");
                        //lbl.addStyleClass("cbx_red");
                        break;
                    case "4":
                        dataModel.setProperty("/valSecThrLvDwr", "keyStlDwr7");
                        iconDWR.setBackgroundColor("");
                        iconDWR.setColor("");
                        //var lbl = view.byId("lbl_SecThreatLvls_dwr_color");
                        //lbl.addStyleClass("cbx_red");
            ...
            InitialFormat: function (digAll, err, decall, name, val, psign, pValue) {
                ...
                var vistaOnChange = this.getView("southern");
                //var oJSONModel = new sap.ui.model.json.JSONModel();
            ...
            OnCbxChng: function (evt) {
                var view = this.getView("southern");
                ...
                MessageBox.confirm("Are you ready to submit?", {
                    ...
                    onClose: async function (sAction) {
                        if (sAction === "OK") {
                            ...
                            var oView = this.that.getView("southern");
            ...
            _onRouteMatched: function (oEvent) {
                ...
                var dataModelSt = this.getOwnerComponent().getModel("dataSt");//Will contain data mapped from the ODATA services
                dataModelSt.oData.init = "X";
                //dataModelInitial = this.getOwnerComponent().getModel("dataOr");
                if (dataModelSt.oData.initidx !== "") {
                    dataModelSt.oData.initidx = "X"
                    this.onInit();
